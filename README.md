...
### Usage
Currently the following are supported (default keys listed; can change in config):
...
As of 1.0.3, there is now a WARP page on the inventory/map UI which allows warping to previously visited save station.  A config option allows turning on unlimited warping without having to visit the save station first.
